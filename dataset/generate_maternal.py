...
num_clients = 10
num_classes =3
dir_path = "maternal_practical_noniid_unbalanced/"
...
def generate_mnist(dir_path, num_clients, num_classes, niid, balance, partition):
    ...
    csv1=pd.read_csv("maternal-main.csv")
    csv1=csv1.drop(["Unnamed: 0"],axis=1)
    csv2=torch.tensor(csv1.drop(["label"],axis=1).values)
    csv3=torch.tensor(csv1["label"])
